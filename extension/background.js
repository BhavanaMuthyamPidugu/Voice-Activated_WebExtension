    ...
    if (request.action === "processCommand") {
        ...
        .then(data => {
            if (data.script) {
                ...
                // Send message to content.js to handle TTS
                chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                    chrome.tabs.sendMessage(tabs[0].id, { action: "speak", text: "Executing: " + request.text });
                });
            } else {
                console.error("No script returned:", data);
            }
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (changeInfo.url) {
        ...
        // Define actions based on the website
        const siteCommands = {
            "youtube.com": ["play video", "pause video", "mute"],
            "twitter.com": ["like tweet", "retweet", "scroll tweets"],
            "google.com": ["search", "open first result"],
            "github.com": ["star repo", "view issues"]
        };
